...
def run(args):
    ...
    midnight = astropy.time.Time(
        datetime.datetime(year, month, day, 12) + datetime.timedelta(hours=19)
    )
    ...
    snapshot, snap_time = fpoffline.io.get_snapshot(
        astropy.time.Time(midnight, format="datetime"), path=args.snap_dir
    )
    ...
    )  # returns an astropy table, not a pandas dataframe.
    summary = (
        metrology[np.isin(metrology["DEVICE_TYPE"], ["POS", "ETC", "FIF", "GIF"])][
            ["LOCATION", "X_FP", "Y_FP"]
        ]
        .group_by("LOCATION")
        .groups.aggregate(np.mean)
    )  # use mean of fiducial locations
    ...
    """
    sql = (
        f"select {expvars} from exposure.exposure where {cond} order by update_time asc"
    )
    ...
    summary.meta["setup_time"] = str(
        exps.query(f"id=={args.setup_id}").iloc[0].update_time
    )
    ...
    if np.isfinite(args.front_id):
        ...
        summary.meta["front_time"] = str(
            exps.query(f"id=={args.front_id}").iloc[0].update_time
        )
    else:
    ...
    if np.isfinite(args.back_id):
        ...
        summary.meta["back_time"] = str(
            exps.query(f"id=={args.back_id}").iloc[0].update_time
        )
    else:
    ...
    if np.isfinite(args.park_id) and args.park_id > args.setup_id:
        ...
        summary.meta["park_time"] = str(
            exps.query(f"id=={args.park_id}").iloc[0].update_time
        )
    else:
    ...
    if args.front_id:
        if args.back_id and (args.back_id - args.front_id != 2):
            logging.warning(
                f"Unexpected back_id - front_id = {args.back_id - args.front_id}"
            )
        ...
        if not front_fits.exists():
            ...
            if args.overwrite or not front_img.exists():
                ...
                fpoffline.fvc.plot_fvc(
                    data, color="cividis", save=front_img, quality=75
                )
    ...
    if args.back_id or args.park_id:
        ...
        if (
            ...
            and args.back_id
            and args.front_id
            and (args.park_id == args.front_id - 2)
        ):
            logging.info(
                f"Using the park back-illuminated expid {args.park_id} instead of {args.back_id}"
            )
            args.back_id = args.park_id
        if args.park_id and not args.back_id:
            logging.info(
                f"Using the park expid {args.park_id} since no final back-illuminated image available."
            )
            args.back_id = args.park_id
        ...
        if not back_fits.exists():
            ...
            if end_time is None and "MJD-OBS" in bhdr and bhdr["MJD-OBS"]:
                ...
                end_time = (
                    astropy.time.Time(bhdr["MJD-OBS"], format="mjd").iso + "+0000"
                )
            ...
            if args.overwrite or not back_img.exists():
                ...
                try:
                    ...
                    fpoffline.fvc.plot_fvc(
                        data, color=(0, 1, 1), save=back_img, quality=85
                    )
                except Exception as e:
                    logging.warning(
                        f"Error reading back-illuminated FVC image {back_fits}"
                    )
                    logging.warning(e)
                    summary.meta["back_id"] = None
                    back_fits = None
        ...
        if back_fits:
            ...
            try:
                ...
                spots = desimeter.processfvc.process_fvc(
                    str(back_fits), use_subprocess=False
                )
                logging.info(f"Fit {len(spots)} spots")
                # Fit the FVC <-> FP transforms to the spots.
                tx = desimeter.transform.fvc2fp.FVC2FP.read_jsonfile(
                    desimeter.io.fvc2fp_filename()
                )
                ...
                dfvc_dx = np.stack(tx.fp2fvc(fp[0] + 0.5, fp[1])) - np.stack(
                    tx.fp2fvc(fp[0] - 0.5, fp[1])
                )
                dfvc_dy = np.stack(tx.fp2fvc(fp[0], fp[1] + 0.5)) - np.stack(
                    tx.fp2fvc(fp[0], fp[1] - 0.5)
                )
                ...
                for petal_loc in range(10):
                    ...
                    summary.meta["keepout"]["gfa"][petal_loc] = [
                        fvcout(xfvc),
                        fvcout(yfvc),
                    ]
                    xfp, yfp = summary.meta["keepout"]["ptl"][petal_loc]
                    xfvc, yfvc = tx.fp2fvc(np.array(xfp), np.array(yfp))
                    summary.meta["keepout"]["ptl"][petal_loc] = [
                        fvcout(xfvc),
                        fvcout(yfvc),
                    ]
            except Exception as e:
    ...
    if end_time is None:
        ...
        logging.warning(
            f"using default end_time {end_time} in absence of any park FVC images"
        )
    ...
    if args.overwrite or not moves_csv.exists():
        ...
        def sum_move(col_in, col_out):
            ...
            angle = (
                ...
                .dropna()
                .str.split("; | ")
                .apply(lambda d: np.sum(np.array(list(map(float, d[1::2])))))
            )
        ...
        x_ptl_from_angles, y_ptl_from_angles = int2ptl(
            moves.pos_t[valid],
            moves.pos_p[valid],
            summary["OFFSET_T"][idx],
            summary["OFFSET_P"][idx],
            summary["LENGTH_R1"][idx],
            summary["LENGTH_R2"][idx],
            summary["OFFSET_X"][idx],
            summary["OFFSET_Y"][idx],
        )
        x_pred, y_pred = ptl2fp_nominal(
            x_ptl_from_angles, y_ptl_from_angles, petal_locs
        )
        ...
        moves.loc[moves.has_fvc_feedback, "fvc_t"] = byloc.pos_t.shift(
            -1, fill_value=np.nan
        )
        moves.loc[moves.has_fvc_feedback, "fvc_p"] = byloc.pos_p.shift(
            -1, fill_value=np.nan
        )
        ...
        moves.drop(
            columns=["fvc_feedback", "has_fvc_feedback", "last_fvc_t", "last_fvc_p"],
            inplace=True,
        )
        # Flag any moves blocked by a comms error or to avoid a collision.
        mask = fpoffline.util.stringToFlag(
            "FROZEN|UNREACHABLE|REJECTED|OVERLAP|EXPERTLIMIT|BOUNDARIES"
        )
        sel = moves.mflags.notna() & moves.log_note.notna()
        anticollide = ~moves[sel].log_note.str.startswith("req_posintTP=") & (
            (moves[sel].mflags & mask) != 0
        )
        badcomms = moves[sel].log_note.str.startswith(
    ...
    last_obs = (
        moves[moves.obs_x.notna() & moves.obs_y.notna()].groupby("location").last()
    )
    ...
    x_ptl, y_ptl = int2ptl(
        summary["POS_T"][valid],
        summary["POS_P"][valid],
        summary["OFFSET_T"][valid],
        summary["OFFSET_P"][valid],
        summary["LENGTH_R1"][valid],
        summary["LENGTH_R2"][valid],
        summary["OFFSET_X"][valid],
        summary["OFFSET_Y"][valid],
    )
    summary["PRED_X"] = np.nan
    summary["PRED_Y"] = np.nan
    summary["PRED_X"][valid], summary["PRED_Y"][valid] = np.round(
        ptl2fp_nominal(x_ptl, y_ptl, summary["PETAL_LOC"][valid]), 6
    )
    ...
    summary["FUNC"] = 0
    summary["FUNC"][
        jdx[snapshot["DEVICE_CLASSIFIED_NONFUNCTIONAL"]]
    ] = 1  # generic non-functional
    summary["FUNC"][jdx[~snapshot["FIBER_INTACT"]]] = 2  # non-functional with bad fiber
    summary["FUNC"][
        summary["FLAGS"] & (1 << 30) != 0
    ] = 3  # non-functional with relay off
    ...
    bit2 = (np.abs(summary["POS_P"] + summary["OFFSET_P"] - 150) > 10) & ~(
        disabled | nonfunc | etc
    )
    summary["INSPECT"][bit2] |= 1 << 2
    groups.append("unparked-P: enabled devices not parked in phi")
    # bit 3 = FP (x,y) does not match angles.
    dxy = np.hypot(
        summary["OBS_X"] - summary["PRED_X"], summary["OBS_Y"] - summary["PRED_Y"]
    )
    ...
    badmatch_sel = moves.log_note.str.startswith(
        "Auto-disabling due to bad match"
    ).fillna(False)
    ...
    bit5 = (
        np.abs(summary["POS_T"])
        >= 360 - summary["PHYSICAL_RANGE_T"] / 2 - theta_hardstop_ambig_tol
    ) & ~(nonfunc | etc)
...
def find_bad_motors(
    ...
    logging.info(
        f"Looking for motors with > {ncut} bad moves (median # of moves is {nmedian})"
    )
    if ncut == ncut_abs:
    ...
    for axis, cut in zip(("theta", "phi"), (tcut, pcut)):
        ...
        sel1 = (
            (moves.ctrl_enabled == 1) & (moves.blocked == 0) & (np.abs(fvc - pos) > cut)
        )
        ...
        for posid in bad_posid:
            ...
            sel2 = (
                ...
                & req.notna()
                & act.notna()
                & (moves.time_recorded >= first_bad.time_recorded)
            )
            # slope, intercept = scipy.stats.siegelslopes(act[sel2], req[sel2])
            _, slope = numpy.polynomial.Polynomial.fit(
                req[sel2], act[sel2], 1, domain=[-1, 1]
            )
            if np.abs(slope - 1) > max_slope_dev:
                logging.info(
                    f"{posid} {axis} has {nsel[posid]} moves that missed by >{cut} deg and slope {slope:.2f} from expid {first_bad.exposure_id}"
                )
...
def int2ptl(t_int, p_int, offset_t, offset_p, length_r1, length_r2, offset_x, offset_y):
    ...
    x_loc, y_loc = desimeter.transform.pos2ptl.ext2loc(
        t_ext, p_ext, length_r1, length_r2
    )
...
def get_calib(DB, at=None, verbose=True):
    ...
    tables = []
    before = (
        "" if at is None else f" where time_recorded<=TIMESTAMP '{pd.Timestamp(at)}'"
    )
    for petal_loc, petal_id in enumerate(fpoffline.const.PETAL_ID_MAP):
...
def main():
    ...
    def hostpath(nersc_path, kpno_path):
        if host == "nersc":
            return pathlib.Path(nersc_path)
        elif host == "kpno":
            return pathlib.Path(kpno_path)
        else:
            return None
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="overwrite any existing output files"
    )
    parser.add_argument(
        "--no-round",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        metavar="PATH",
        default=hostpath("/global/cfs/cdirs/desi/spectro/data", "/exposures/desi"),
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        default=hostpath(
            "/global/cfs/cdirs/desi/engineering/focalplane/calibration",
            "/data/focalplane/calibration",
        ),
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        default=hostpath(
            "/global/cfs/cdirs/desi/engineering/focalplane/PositionerIndexTable/index_files",
            "/software/products/PositionerIndexTable-trunk/index_files",
        ),
    ...
    parser.add_argument(
        ...
        metavar="YAML",
        default=hostpath(
            "/global/cfs/cdirs/desi/engineering/focalplane/db.yaml",
